...
@Component
public class SeniorityManagerStrategy implements SeniorityStrategy {
    @Override
    public boolean matchBySeniority(SeniorityEnum seniorityEnum) {
        return SeniorityEnum.MANAGER.equals(seniorityEnum);
    }

    @Override
    public Response calculateSalaryBonus(BigDecimal salary) {
        var salaryIncrease = salary.add(SeniorityEnum.MANAGER.calculateSalaryBonus(salary));
